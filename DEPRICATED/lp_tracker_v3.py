...
import asyncio
import re
from pyodide.http import pyfetch #req deleted
from js import console

# optimiert fuer Input aus HTML

api_key = 'RGAPI-de29b497-f274-47f4-8fd8-a30d2645530a'
...
def get_input_name(): # Methode zur Eingabe des Namens durch Nutzer
    ...
    while(True):
        ...
        inp = Element('inputText').element.value
        console.log('input in Python' + inp)
        if inp == 'skip':
            split_name = 'skip'
            break
        elif inp == 'end':
...
async def does_acc_exist(name, tag): # checkt, ob eingegebener Name ueberhaupt exisitert (aehnlich zu find_acc_uuid)
    region = 'europe'
    req_url = (
        'https://' +
        region +
    ...
    player_get = await pyfetch(url=req_url, method="GET")
    player_info = player_get.json()
    if 'status' in player_info.keys(): # falls Name nicht existiert steht in der json der FehlerSTATUS
        return False
    else:
        return True

async def find_acc_uuid(name, tag): # TODO
...
async def find_summoner_id(puuid):
    req_url = (
...
async def find_rank_info(summoner_id): # return als Dict mit String : Liste von Listen {'solo': [Rang], 'flex': [Rang]}
    req_url = (
    ...
    player_get = await pyfetch(url=req_url, method="GET")
    p_info = player_get.json() # legt nur Eintrag an, falls man eingeranked ist
...
def main():
    console.log('KONSOLE GESTARTET')
    players = []
    while(True):
        input_name = get_input_name()
        if input_name == 'end':
            break # beende Programm
        elif input_name != 'skip':
        ...
        for p in players:
            ...
            p.update_rank()
            print('')
            print('Spieler: ' + str(p.get_name()))

        update_json(players, r'LP_Tracker_JS\player_list.json')
